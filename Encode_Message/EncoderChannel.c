...
int encodeCharacters2(bitstream_t* stream, exi_string_character_t* chars, size_t len) {
        unsigned int i;
        int errn = 0;
        for (i = 0; i < len && errn == 0; i++) {
#if STRING_REPRESENTATION == STRING_REPRESENTATION_ASCII
                // errn = encode(stream, (uint8_t)chars[i]);
		 stream->data[(*stream->pos)++] = (uint8_t)chars[i];
#endif /* STRING_REPRESENTATION_ASCII */
#if STRING_REPRESENTATION == STRING_REPRESENTATION_UCS
                //errn = encodeUnsignedInteger32(stream, chars[i]);
                errn = encode(stream, (uint8_t)chars[i]);
 ...
 stream->data[(*stream->pos)++] = 0X02;
        return errn;
}



int writeChars(bitstream_t* stream, size_t nbits, uint32_t val) {
    int errn = 0;
    // Loop through each bit of the ASCII character
    for (int i = (nbits - 1); i >= 0; i--) {
        // Extract each bit from the ASCII character
        uint32_t bit = (val >> i) & 1;
        // Write the bit to the stream
#if EXI_STREAM == BYTE_ARRAY
        // Write the bit to the byte array at the current position
        if ((*stream->pos) >= stream->size) {
            errn = EXI_ERROR_OUTPUT_STREAM_EOF;
            break;
        }
        stream->data[(*stream->pos)] |= (bit << (stream->capacity - 1));
        // Move to the next position in the byte array
        (*stream->pos)++;
#elif EXI_STREAM == FILE_STREAM
        // Write the bit to the file associated with the stream
        if (putc((int)bit, stream->file) == EOF) {
            errn = EXI_ERROR_OUTPUT_STREAM_EOF;
            break;
        }
#endif
        ...
        if (stream->capacity == 0) {
            // Reset the buffer and update the capacity
